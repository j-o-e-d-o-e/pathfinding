...
public class PubBeach extends Pub {

    public PubBeach(GameMain game) {
        super(game);
        if (GameManager.storyStage == 1) {
            message.setText(name + " enters the pub. No sign of \"Cottonmouth\".");
        }
    ...
    void chatWithAGuest() {
        ...
        } else if (GameManager.storyStage == 1 && ((beerCount >= 1 && beerCount < 3) || mealCount >= 1)) {
            message.setText("");
    ...
    void handleStoryInput() {
        ...
        } else if (GameManager.storyStage == 1 && Gdx.input.isKeyJustPressed(Keys.ANY_KEY)) {
            hud.removeStory();
    ...
    public void render(float delta) {
        ...
        game.getBatch().begin();
        title.draw(game.getBatch(), "BEACH PUB", GameInfo.WIDTH / 2f - 250, GameInfo.HEIGHT / 2f + 200);
        game.getBatch().end();
